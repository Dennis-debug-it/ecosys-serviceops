...
function renderEmailNotificationFields(current: UpdateEmailNotificationSettingsInput, setForm: (value: UpdateEmailNotificationSettingsInput) => void, hasSecret: boolean) {
  ...
  return <>
    ...
    <Field label="SMTP port"><input disabled={!smtpMode} type="number" value={current.smtpPort} onChange={(event) => update('smtpPort', Number(event.target.value) || 0)} className="field-input" /></Field>
